...
public class Model2B_Rev1Impl {
	...
	private void i2c() throws IOException {
		try	{
			final int ADS1015_ID = ADS1015GpioProvider.ADS1015_ADDRESS_0x48;
			final int DEVICE_ID = ADS1015_ID;
			final Pin ADS1015Pins[] =  {ADS1015Pin.INPUT_A0, ADS1015Pin.INPUT_A1, ADS1015Pin.INPUT_A2, ADS1015Pin.INPUT_A3};
			 // create gpio controller
	        final GpioController gpio = GpioFactory.getInstance();
			
			// create custom ADS1015 GPIO provider
			final ADS1015GpioProvider gpioProvider = new ADS1015GpioProvider(I2CBus.BUS_1, DEVICE_ID);
	       ...
	       gpioProvider.setProgrammableGainAmplifier(ProgrammableGainAmplifierValue.PGA_4_096V, ADS1015Pin.ALL);
	        ...
	        gpioProvider.setEventThreshold(1, ADS1015Pin.ALL);
	       ...
	       //give ADS time to setup
	       Thread.sleep(1000);

	       //provision gpio analog input pins from ADS1015
	       final GpioPinAnalogInput I2C_Pin = gpio.provisionAnalogInputPin(gpioProvider, ADS1015Pins[0], "A0");
	       
	       AnalogSensorComponent sc = new AnalogSensorComponent(I2C_Pin);
	       osgi.enroute.iot.pi.provider.I2C i2c = new osgi.enroute.iot.pi.provider.I2C("I2C_A0", board, dtos);
	       
			
			I2C_Pin.setName("I2C");
			I2C_Pin.addListener(new GpioPinListenerAnalog() {
				
				@Override
				public void handleGpioPinAnalogValueChangeEvent(GpioPinAnalogValueChangeEvent event) {
					//take 20 readings and then take the average to minimize noise
	        		double reading = 0;
	        		for(int j = 0; j < 20; ++j) {
			        	reading += gpioProvider.getValue(ADS1015Pin.INPUT_A0);
			        	try {
		        	...
		        	double average = reading / 20;
		        	
					i2c.set(average, 0);
					System.out.println("I2CValue: " + osgi.enroute.iot.pi.provider.I2C.getValue(0));
					board.fire(i2c, "set", average);
				}
			});
			
			
			//TODO: Remove
			System.out.println(I2C_Pin.getPin());
			
			//register pin
			register(RaspiPin.GPIO_08, osgi.enroute.iot.pi.provider.I2C.class, i2c, I2C_Pin, "I2C_A0");
			board.fire(i2c, "set", I2C_Pin.getValue());
